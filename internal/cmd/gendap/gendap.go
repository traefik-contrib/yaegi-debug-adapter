...
func main() {
	...
	_, _ = fmt.Fprintf(w, "\n")
	_, _ = fmt.Fprintf(w, "// Code generated by 'go run ../internal/cmd/gendap'. DO NOT EDIT.\n")
	_, _ = fmt.Fprintf(w, "\n")
	_, _ = fmt.Fprintf(w, "import \"fmt\"\n")
	_, _ = fmt.Fprintf(w, "\n")
	_, _ = fmt.Fprintf(w, "type RequestArguments interface{ requestType() string }\n")
	_, _ = fmt.Fprintf(w, "type ResponseBody interface{ responseType() string }\n")
	_, _ = fmt.Fprintf(w, "type EventBody interface{ eventType() string }\n")
	_, _ = fmt.Fprintf(w, "\n")
...
func (t dapTypes) writeFuncs(w io.Writer, name string) {
	for _, t := range t {
		_, _ = fmt.Fprintf(w, "func (*%s) %s() string { return %q }\n", t.Name, name, t.Identifier)
	}
	_, _ = fmt.Fprintf(w, "\n")
}
...
func (t dapTypes) writeConstructor(w io.Writer, name, typ, err string) {
	_, _ = fmt.Fprintf(w, "func %s(x string) (%s, error) {\n", name, typ)
	_, _ = fmt.Fprintf(w, "\tswitch x {\n")
	for _, t := range t {
		_, _ = fmt.Fprintf(w, "\tcase %q:\n", t.Identifier)
		_, _ = fmt.Fprintf(w, "\t\treturn new(%s), nil\n", t.Name)
	}
	_, _ = fmt.Fprintf(w, "\tdefault:\n")
	_, _ = fmt.Fprintf(w, "\t\treturn nil, fmt.Errorf(%q, x)\n", err)
	_, _ = fmt.Fprintf(w, "\t}\n")
	_, _ = fmt.Fprintf(w, "}\n")
	_, _ = fmt.Fprintf(w, "\n")
}
